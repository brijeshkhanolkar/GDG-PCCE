...
function predictEta(inputCase) {
  ...
  if (nearestCluster && nearestCluster.disposedCount >= 15) {
    ...
    // Stage comparison metrics for Feature 5
    const stageIndex = stages.indexOf(inputCase.current_stage);
    let stageComparison = null;
    if (inputCase.current_stage !== 'Disposed' && stageIndex >= 0) {
      const totalMedianMonths = Math.max(6, Math.round(median * 12));
      const avgStageMonths = Math.max(2, Math.round(totalMedianMonths / 5.5));
      const estCurrentStageMonths = Math.max(1, Math.round((inputCase.days_elapsed / (stageIndex + 1)) / 30.4));
      stageComparison = {
        currentStage: inputCase.current_stage,
        timeAtCurrentStageMonths: estCurrentStageMonths,
        avgTimeAtStageForClusterMonths: avgStageMonths,
      };
    }

    // Explanatory ETA Factors for Feature 3
    const medianYears = Math.round(median * 10) / 10;
    const avgAdj = Math.round(avgAdjournments * 10) / 10;
    const etaFactors = [
      `${inputCase.case_type} cases in this cluster typically take ${medianYears} years`,
      inputCase.num_adjournments > avgAdj + 0.8
        ? `This case has ${inputCase.num_adjournments} adjournments, above the ${avgAdj} average for similar cases — this tends to add time.`
        : inputCase.num_adjournments < avgAdj - 0.8
          ? `This case has fewer adjournments (${inputCase.num_adjournments}) than typical for its cluster (avg ${avgAdj}).`
          : `This case has ${inputCase.num_adjournments} adjournments, consistent with the cluster baseline (${avgAdj}).`,
      inputCase.current_stage === 'Disposed'
        ? `Case has completed all procedural stages and reached final disposition.`
        : `${inputCase.current_stage} is historically one of the slower stages for this case type.`,
      `Based on ${nearestCluster.size.toLocaleString()} similar historical cases`,
    ];
  ...
  const etaFactors = [
    `${inputCase.case_type} cases in this cohort typically take ${Math.round(median * 10) / 10} years`,
    inputCase.num_adjournments > avgAdj + 0.8
      ? `This case has ${inputCase.num_adjournments} adjournments, above the ${avgAdj} average for similar cases — this tends to add time.`
      : inputCase.num_adjournments < avgAdj - 0.8
        ? `This case has fewer adjournments (${inputCase.num_adjournments}) than typical for its cohort (avg ${avgAdj}).`
        : `This case has ${inputCase.num_adjournments} adjournments, consistent with cohort average (${avgAdj}).`,
    inputCase.current_stage === 'Disposed'
      ? `Case has completed all procedural stages and reached final disposition.`
      : `${inputCase.current_stage} is historically one of the slower stages for this case type.`,
    `Based on ${pool.length.toLocaleString()} similar historical cases`,
  ];
